...
Prompt the user to save raw, real app captures in `assets/screenshots/source/`. If a running build, emulator, or device is available, help capture the real flows there. Otherwise, identify the exact screens to capture and wait for actual captures. Do not generate or submit synthetic screenshots, invented UI, or edited screenshots that misrepresent the app's current experience.

Use [scripts/render_store_screenshots.py](scripts/render_store_screenshots.py) to make non-destructive, exact-size exports from user-provided captures. Use `direct` mode for faithful store-ready captures. Use `marketing` mode only when requested or when the user asks for a complete marketing treatment; it may add an honest headline and branded canvas while preserving the whole captured UI. The script does not create device frames. It must not crop a materially mismatched direct capture unless the user explicitly authorizes cropping after seeing the consequence. Preserve source images, use a new output name unless replacement is explicitly requested, and run validation after rendering.

Use image generation for missing supporting marketing artwork when the user has asked for assets and sufficient app/brand evidence exists. In particular, it can draft a Google Play feature graphic or a non-UI background used by marketing mode. Base prompts on verified app facts and provided brand materials, inspect the result, then save the selected final artifact in the checklist's destination without overwriting user work. Never use image generation to replace, alter, or fabricate the actual app interface in a store screenshot.
